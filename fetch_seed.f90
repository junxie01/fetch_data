         ...
         if (.not.ext)then
            ...
            if(do_mini.eq.1)then
               if(do_resp.eq.0)then
                  write(command,'("FetchData -b",1x,1a,1x,"-o",1x,1a,1x,"-m",1x,1a,1x,"-rd resp/resp_",i4.4,"_",i3.3)')&
                          trim(mail),trim(output_seed),trim(mdata),iyear,jday
               else
                  !write(command,'("FetchData -b",1x,1a14,1x,"-rd resp/resp_",i4.4,"_",i3.3)')trim(mail),iyear,jday
                  write(command,'("FetchData -b",1x,1a14,1x,"-rd resp/resp_",i4.4,"_",i3.3,1x," -m&
               ",1x,1a)')trim(mail),iyear,jday,trim(mdata)
               endif
               call system(command)
