...
parameter (nn=21048576,neqmax=3000,nstmax=1000)
...
do i=1,nstmax
   read(13,*,end=9,err=9)netlist(i),stalist(i),tmp
enddo
9 close(13) 
n_sta=i !number of station
write(*,*)'n_sta=',n_sta
...
!   if(ext)cycle
   ...
   enddo
   close(12)
   write(command,'("mkdir -p resp/",i4.4,"_",i2.2,"_",i2.2,1x,"2>/dev/null")')year,month,day
   call system(command)
   if(do_resp.eq.0)then
      write(command,'("FetchData -b",1x,1a,1x,"-o",1x,1a,1x,"-m",1x,1a,1x,"-rd resp/",i4.4,"_",i2.2,"_",i2.2)')&
      trim(mail),trim(output_seed),trim(mdata),year,month,day
   else
      write(command,'("FetchData -b",1x,1a,1x,"-rd resp/",i4.4,"_",i2.2,"_",i2.2)')trim(mail),year,month,day
