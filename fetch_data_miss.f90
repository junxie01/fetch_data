...
call system("mkdir seed_miss 2>/dev/null")
call system("mkdir resp_miss 2>/dev/null")
call system("mkdir mdat_miss 2>/dev/null")
...
!         write(*,*)'output_seed is',output_seed
         if (.not.ext)then
            ...
            write(seed,'("seed_miss/",i4.4,"_",i3.3,".seed")')iyear,jday
            write(mdata,'("mdat_miss/",i4.4,"_",i3.3,".mdata")')iyear,jday
            ...
            write(12,'(".END")')
            write(12,'("")')
            write(sta_list,'("stat_miss/",i4.4,"_",i3.3,".lst")')iyear,jday
...
!write(*,*)'n_sta=',n_sta
            ...
            enddo
            close(12)
            write(command,'("mkdir -p resp_miss/resp_",i4.4,"_",i3.3,1x,"2>/dev/null")')iyear,jday
            call system(command)
            if (do_resp.eq.0)then
               write(command,'("FetchData -b",1x,1a,1x,"-o",1x,1a,1x,"-m&
               ",1x,1a,1x,"-rd resp_miss/resp_",i4.4,"_",i3.3)')trim(mail),trim(output_seed),trim(mdata),iyear,jday
            else ! download response file only
